...
The processes below assume that the V80 has already been successfully programmed with a `Vollo Trees` image at least once,
either via JTAG or PCIe. If you have not yet programmed the card with a `Vollo Trees` image, please follow the instructions
...
Running the full `lspci` command should display the V80 card loaded with a `Vollo Trees` image as below. The card's BDF here is `01:00`.
